...
pub fn decode(encoded: &[u8]) -> (Result<BencodeObj, ()>, &[u8]/* remaining */) {
    ...
    for ch in encoded.iter() {
        match state {
            State::Ready => {
                match ch {
                    b'0'..=b'9' => { state = State::RdStrLen; end_pointer += 1; },
                    b'i'       => { state = State::RdInt; start_pointer += 1; },
                    b'l'       => { state = State::RdLst; start_pointer += 1; },
                    b'd'       => { state = State::RdDic; start_pointer += 1; },
                    _         => { unexpected = true; }
                }
            },
    ...
    match state {
        ...
        State::RdStrCon => {
            return (
                Ok(BencodeObj::Str(encoded[start_pointer..end_pointer].to_vec())),
                &encoded[end_pointer..]
            );
        },
        State::RdLst => {
